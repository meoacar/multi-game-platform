...
<div class="container mx-auto px-4 py-6">
    <div class="max-w-3xl mx-auto">
        <!-- Header -->
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900">Rozet Düzenle</h1>
            <p class="text-gray-600 mt-1">{{ $badge->name }} rozetini düzenleyin</p>
        </div>

        <!-- Form -->
        <form action="{{ route('admin.badges.update', $badge) }}" method="POST" class="bg-white rounded-lg shadow-sm p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <input type="text" name="slug" value="{{ old('slug', $badge->slug) }}" required
            ...
            <div class="mb-4">
                ...
                <textarea name="description" rows="3" required
                          class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $badge->description) }}</textarea>
                @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="category" required
                    ...
                    <option value="gameplay" {{ old('category', $badge->category) == 'gameplay' ? 'selected' : '' }}>Oyun</option>
                    <option value="social" {{ old('category', $badge->category) == 'social' ? 'selected' : '' }}>Sosyal</option>
                    <option value="content" {{ old('category', $badge->category) == 'content' ? 'selected' : '' }}>İçerik</option>
                    <option value="special" {{ old('category', $badge->category) == 'special' ? 'selected' : '' }}>Özel</option>
                    <option value="activity" {{ old('category', $badge->category) == 'activity' ? 'selected' : '' }}>Aktivite</option>
                    <option value="moderation" {{ old('category', $badge->category) == 'moderation' ? 'selected' : '' }}>Moderasyon</option>
                </select>
                @error('category')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <select name="rarity" required
                    ...
                    <option value="">Seçiniz</option>
                    <option value="common" {{ old('rarity', $badge->rarity) == 'common' ? 'selected' : '' }}>Yaygın</option>
                    <option value="rare" {{ old('rarity', $badge->rarity) == 'rare' ? 'selected' : '' }}>Nadir</option>
                    <option value="epic" {{ old('rarity', $badge->rarity) == 'epic' ? 'selected' : '' }}>Epik</option>
                    <option value="legendary" {{ old('rarity', $badge->rarity) == 'legendary' ? 'selected' : '' }}>Efsanevi</option>
                </select>
                @error('rarity')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <input type="number" name="xp_required" value="{{ old('xp_required', $badge->xp_required) }}" min="0"
            ...
            <div class="mb-4">
                ...
                <input type="number" name="sort_order" value="{{ old('sort_order', $badge->sort_order) }}" min="0"
            ...
            <div class="mb-4 space-y-2">
                <label class="flex items-center">
                    <input type="checkbox" name="is_hidden" value="1" {{ old('is_hidden', $badge->is_hidden) ? 'checked' : '' }}
